...
class GuitarrasRepositoryImplTest {
    ...
    @BeforeAll
    fun setUpAll() {
        // Borramos la base de datos, pero ya esta en memoria
    ...
    fun findById() {
        ...
        val guitarra = repository.save(getGuitarraInitData().first())

        val find = repository.findById(guitarra.id)

        assertAll(
            { assertNotNull(find) },
            { assertEquals("Gibson", find?.marca) },
            { assertEquals("Les Paul Standard", find?.modelo) },
            { assertEquals(2333.0, find?.precio) },
            { assertEquals(10, find?.stock) }
        )
    }
